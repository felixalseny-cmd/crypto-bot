...
const checkExpiredSubscriptions = async () => {
  ...
  try {
    ...
    console.log('Found expired subscriptions:', expiredUsers.length);

    for (const user of expiredUsers) {
      try {
        console.log('Removing user from VIP channel:', user.userId);
        ...
        try {
          ...
          console.log('Successfully removed user from VIP channel:', user.userId);
        } catch (banError) {
          console.log('Could not remove user from channel:', user.userId, banError.message);
        }
        ...
        try {
          await bot.sendMessage(user.userId,
            'Your VIP subscription has expired. To continue receiving premium signals, please renew your subscription.',
            {
              reply_markup: {
                inline_keyboard: [
          ...
          console.log('Could not send expiration message to user:', user.userId);
        }
...
async function addToVipChannel(chatId, plan) {
  try {
    console.log('Adding user to VIP channel:', chatId);
    
    await bot.addChatMember(process.env.VIP_CHANNEL_ID, chatId);
    
    console.log('Successfully added user to VIP channel:', chatId);
    ...
    console.error('Failed to add user to VIP channel:', chatId, error.message);
    
    if (error.response && error.response.body &&
        error.response.body.description &&
        error.response.body.description.includes('USER_ALREADY_PARTICIPANT')) {
      console.log('User is already in VIP channel:', chatId);
      await User.findOneAndUpdate(
        { userId: chatId },
        {
          inVipChannel: true,
      ...
      try {
        ...
        if (user && user.pendingPayment) {
          ...
          if (addedToChannel) {
            ...
            await bot.sendMessage(chatId,
              `Payment Verified!\n\nYour ${plan} VIP subscription has been activated!\n\nCould not automatically add you to VIP channel. Please contact support.`
            );
          }
        }
      } catch (error) {
