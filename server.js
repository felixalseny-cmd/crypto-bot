...
const fetch = require('node-fetch'); // Добавим для TON API
const app = express();
...
async function notifyAdmin(txHash, userId, username, amount, currency, plan) {
  const adminId = process.env.ADMIN_TELEGRAM_ID; // Обязательно задайте в .env
  if (!adminId) return;
  const message = `
...
async function verifyTONTransaction(txHash, expectedAmount, walletAddress) {
  try {
    ...
    const expected = walletAddress.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
    if (!dest || dest.toLowerCase() !== expected) return false;
...
bot.on('message', async (msg) => {
  ...
  // Для USDT/USDC — только уведомление админу
  await User.findOneAndUpdate(
    { userId: chatId },
    {
      $push: {
