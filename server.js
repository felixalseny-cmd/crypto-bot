...
function setupBotHandlers() {
  const sendStartMessage = async (chatId, firstName) => {
    const welcomeMessage = `🚀 <b>Welcome to FXWave VIP Access, ${firstName}!</b>\n\nChoose your subscription plan:`;
    const options = {
      parse_mode: 'HTML',
      reply_markup: {
        inline_keyboard: [
  ...
  async function sendPaymentInstructions(chatId, plan) {
    ...
    const message = `💳 <b>Payment Instructions for ${plan.toUpperCase()}</b>\n\n` +
  ...
  async function showUserSubscription(chatId) {
    ...
    await bot.sendMessage(chatId,
      ...
      `⏰ Expires in: <b>${remainingTime} days</b>\n` +
      `📅 Renewal: <b>${user.expiresAt.toLocaleDateString()}</b>\n` +
      `🎯 VIP Access: <b>${user.inVipChannel ? 'Active' : 'Pending'}</b>`,
      {
        parse_mode: 'HTML',
        reply_markup: {
          inline_keyboard: [
  ...
  async function sendHowToPay(chatId) {
    const message = `💡 <b>How to Pay with USDT</b>\n\n` +
  ...
  bot.on('message', async (msg) => {
    ...
    await bot.sendMessage(chatId,
      `⏳ <b>Transaction received!</b> Verifying hash: <code>${msg.text.substring(0, 12)}...</code>\n\nThis may take a few minutes.`,
      { parse_mode: 'HTML' }
    );
    ...
    setTimeout(async () => {
      try {
        ...
        if (user && user.pendingPayment) {
          ...
          if (added) {
            await bot.sendMessage(chatId,
              `✅ <b>Payment Verified!</b>\n\nYour <b>${plan}</b> VIP subscription has been activated!\n\n🎉 <b>You have been automatically added to the VIP channel!</b>`,
              { parse_mode: 'HTML' }
            );
          } else {
            await bot.sendMessage(chatId,
