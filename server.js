...
bot.on('callback_query', async (callbackQuery) => {
  ...
  try {
    if (data.startsWith('select_plan_')) {
      ...
      if (tonAvailable) {
        currencyButtons.push([{ text: '🪙 TON', callback_data: `pay_TON_${plan}` }]);
      }
      if (usdtAvailable) {
        currencyButtons.push([{ text: '💵 USDT (TRC20)', callback_data: `pay_USDT_${plan}` }]);
      }
      // 🔹 Добавьте USDC здесь, если захотите (адрес тот же)
      // if (usdtAvailable) {
      //   currencyButtons.push([{ text: '🔵 USDC (TRC20)', callback_data: `pay_USDC_${plan}` }]);
      // }
      ...
      currencyButtons.push([{ text: '🔙 Back', callback_data: 'back_to_start' }]);
      await bot.editMessageText(
...
// 🧪 /testchannel — для проверки прав бота
...
app.get('/qr', async (req, res) => {
  ...
  const wallet = currency === 'TON' ? process.env.TON_WALLET_ADDRESS : process.env.USDT_WALLET_ADDRESS;
  if (!wallet) {
    return res.status(400).send('Payment method temporarily unavailable');
  }
  const prices = { USDT: { '1month': 24, '3months': 55 }, TON: { '1month': 11, '3months': 25 } };
  ...
  if (currency === 'TON') {
    const nanoTons = Math.round(amount * 1e9);
    data = `ton://transfer/${wallet}?amount=${nanoTons}`;
  } else {
